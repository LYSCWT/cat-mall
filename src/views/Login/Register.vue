...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      // 校验验证码+更新数据库
      loginApi.insertUser({name:ruleForm.userName,password:ruleForm.pass,phone:ruleForm.phone}).then(res=>{
        if(res){
          ElMessage.success({message:"注册成功！请重新登录！",duration:1000})
          //跳转到登录页面
          setTimeout(()=>{
...
<template>
  ...
  <div class="context">
    ...
    >
      ...
      <el-form-item>
        <el-button type="primary" @click="submitForm(ruleFormRef)" class="su-bu">注册</el-button>
